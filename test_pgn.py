...
parser.add_argument(
    "--DATASET",
    type=str,
    default="selected_dmd_512")
args = parser.parse_args()

os.chdir(os.path.dirname(os.path.abspath(__file__)))

print("Now in the dataset {}".format(args.DATASET))
N_CLASSES = 20
...
parsing_dir = f'./output/{DATASET}/cihp_parsing_maps'
edge_dir = f'./output/{DATASET}/cihp_edge_maps'
...
def main():
    ...
    with tf.name_scope("create_inputs"):
        reader = ImageReader(DATA_DIR, LIST_PATH, DATA_ID_LIST,
                             None, False, False, False, coord)
    ...
    edge_out2_100 = tf.image.resize_images(
        edge_out2_100, tf.shape(image_batch)[1:3,])
    edge_out2_125 = tf.image.resize_images(
        edge_out2_125, tf.shape(image_batch)[1:3,])
    edge_out2_150 = tf.image.resize_images(
        edge_out2_150, tf.shape(image_batch)[1:3,])
    edge_out2_175 = tf.image.resize_images(
        edge_out2_175, tf.shape(image_batch)[1:3,])
    edge_out2 = tf.reduce_mean(
        tf.stack([edge_out2_100, edge_out2_125, edge_out2_150, edge_out2_175]), axis=0)
